...
cdef double initial_dbl[1]
cdef int initial_int[1][3]
...
def animate(i):
	...
	# Set the new time
	time_text.set_text("Time = {:2.3f}, Num prtcls: {}".format(ani_time, cur_dim))
...
fig.suptitle("Branching Random Walk in a Random Environment")
ax1       = fig.add_subplot(1,2,1)
ax1.set_title("Particle System")
...
im1       = ax1.imshow(brwre_histo*(B_NUM_EXTRA/DIM_BOX)**2, interpolation='none', origin='low', vmin = 0, vmax = 0.3*B_NUM_EXTRA,  \
...
im2       = ax2.imshow(noise_avrg, interpolation='none', origin='low', vmin = -0.05*intensity*B_NUM_averaging, vmax = 0.05*intensity*B_NUM_averaging,  \
...
ani       = animation.FuncAnimation(fig, animate, frames=600, interval = 60, blit = True)
ani.save(filename = 'brwre_2D.mp4', extra_args=['-vcodec', 'libx264'], bitrate = 20000)
